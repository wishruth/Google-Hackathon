...
class EfficientDetDetector(
    ...
    labelAssetName: String = LABEL_ASSET_NAME,
    confidenceThreshold: Float = 0.35f,
) : AutoCloseable {
    private val appContext = context.applicationContext
    private val labels = EfficientDetLabelProvider.loadLabels(appContext, labelAssetName)
    ...
    private fun runModel(model: ActiveModel, preprocessed: PreprocessedFrame): List<BoundingBox> {
        ...
        return postprocessor.process(outputArrays, preprocessed.metadata).map { it.toBoundingBox() }
    }
    ...
    private companion object {
        ...
        const val LABEL_ASSET_NAME = "labels.txt"
